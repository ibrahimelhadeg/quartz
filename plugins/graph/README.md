...
Local fork of [`@quartz-community/graph`](https://github.com/quartz-community/graph) for this repo. Tracks upstream; layers **local-graph** customizations (E4) on top:

- **Readable always-on labels** — `showLabels` keeps every node title visible without hover/zoom (paired with config `fontSize` / `opacityScale` / `scale`).
- **Relationship-kind edges** — each edge is coloured/styled by its relation to the current page: outgoing (this page → neighbour, blue + arrow), incoming backlink (neighbour → this page, amber + arrow), shared-tag co-membership (dashed, tag accent).
- **Legend** — `legend` renders a corner key so the encoding is discoverable from the page.

Built to `dist/` via `node build.mjs` (esbuild + sass, mirrors upstream `tsup.config.ts`); the host loader uses the prebuilt `dist/` directly. The global graph is unchanged (E5 named clusters is future work).
